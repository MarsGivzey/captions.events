...
interface OBSViewPageProps {
  ...
  searchParams: Promise<{ size?: string; color?: string; position?: string }>
}

export default async function OBSViewPage({ params, searchParams }: OBSViewPageProps) {
  const { uid } = await params
  const { size = "lg", color = "white", position = "bottom" } = await searchParams
  ...
  return <OBSViewer event={event} size={size} color={color} position={position} />
}
